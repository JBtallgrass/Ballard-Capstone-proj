...
This section applies supervised and unsupervised learning techniques to evaluate and surface patterns of structural vulnerability across Arkansas counties. The goal is twofold: first, to assess how core indicators such as education and employment predict economic hardship; and second, to group counties into data-driven risk profiles for targeted regional planning.

\subsection{Linear Regression Models} \label{subsec:linear_regression}

To establish interpretable relationships among key socioeconomic indicators, we first employed multiple linear regression. Two models were developed: one to predict county-level \textbf{Poverty Rate}, and another to assess predictors of the engineered \textbf{Vulnerability Score} introduced in Section~\ref{sec:feature_engineering}.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Capstone_Project_Report/images/linear_regression_diagnostics.png}
...
These regression models lay the groundwork for unsupervised clustering by confirming that the variables used in risk profiling are both statistically and substantively meaningful.

\subsection{Clustering Analysis and Latent Risk Profiles} \label{subsec:clustering}

To uncover hidden patterns of vulnerability, KMeans clustering was applied using the engineered and scaled features. This unsupervised method grouped counties into latent profiles based on educational attainment, poverty, unemployment, and education gaps. Unlike regression, clustering does not rely on predefined labels, making it well-suited for exploratory regional segmentation.

Silhouette analysis revealed that $k=2$ provided the best fit, yielding two clusters interpreted as:
