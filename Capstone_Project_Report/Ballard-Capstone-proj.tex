...
Certainly. Here's a refined version of your subsection that expands on each point with narrative flow and academic tone, while avoiding the use of a bullet list:
...
Yes, this code is present in your `capstone_analyticsvAR.ipynb` file, specifically in the section where you prepare features for modeling and apply normalization using `MinMaxScaler`. It aligns exactly with your `features_to_scale` list and scaling logic.

Here’s an expanded version of your paragraph that contextualizes and elaborates on the rationale and impact of scaling, while preserving the technical tone:
...
The EducationIndex was computed by averaging the scaled values of bachelor’s degree attainment and high school graduation rates. This combined metric offered a broader view of educational conditions, capturing both foundational and postsecondary attainment levels. Similarly, the EconomicIndex was derived by averaging the scaled poverty and unemployment rates—two dimensions of economic hardship that often intersect and reinforce one another. By aggregating these values, the EconomicIndex provided a more nuanced depiction of economic strain than any single indicator alone.

The final VulnerabilityIndex was calculated by averaging the EducationIndex and EconomicIndex, resulting in a continuous variable that represented multidimensional socioeconomic risk. This integrated score enabled each county to be positioned along a unified vulnerability spectrum, facilitating comparisons and informing cluster analysis.

Visualizing the VulnerabilityIndex as a choropleth map (Figure~\ref{fig:vulnerability_heatmap}) revealed striking regional disparities. Counties with elevated vulnerability scores tended to cluster in the Mississippi Delta and portions of North Central Arkansas—areas historically affected by underinvestment, low attainment, and economic stagnation. The map served not only as a diagnostic tool for identifying priority areas but also as a communication artifact that could guide policymakers, educators, and workforce leaders toward targeted interventions.
...
To interpret the underlying structure of the clusters, a series of visualizations were created to examine how counties grouped along key socioeconomic dimensions. These scatterplots reveal how cluster assignments relate to observed disparities in education, poverty, unemployment, and population characteristics. Together, they provide a multifaceted view of the latent patterns surfaced by the KMeans algorithm.
...
However, it is important to interpret these results with caution, especially given the small size of the test set. Although the absence of misclassifications is encouraging, model generalizability should be evaluated on larger or out-of-sample datasets in future work to ensure long-term robustness.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Capstone_Project_Report/images/maps/images_feature_importance_analysis.png}
\caption{Feature Importance (Decision Tree Classifier): Educational indicators dominate the classification logic, confirming the foundational role of postsecondary attainment in county risk profiles.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{Capstone_Project_Report/images/maps/final_confusion_matrix.png}
\caption{Confusion Matrix (Decision Tree): All counties in the test set were correctly classified into their respective clusters.}
...
\caption{Random Forest Feature Importance Comparison: Educational gap and bachelor’s degree percentage remain central even in ensemble-based models, reinforcing their predictive significance across techniques.}
