...
\begin{itemize}
  \item \textbf{A.} Data Dictionary
  \item \textbf{B.} Code/Model Summaries
  \item \textbf{C.} County-Level Snapshots
  \item \textbf{D.} Methodological Notes
  \item \textbf{E.} North Central Arkansas Counties
\end{itemize}
...
\section{Code and Model Summaries}
\label{appendix:code_model}

\begin{lstlisting}[language=Python, caption={Random Forest Model Initialization}]
from sklearn.ensemble import RandomForestClassifier
model = RandomForestClassifier(n_estimators=100, random_state=42, max_depth=5)
model.fit(X_train, y_train)
\end{lstlisting}

\begin{lstlisting}[language=Python, caption={KMeans Clustering Setup}]
from sklearn.cluster import KMeans
kmeans = KMeans(n_clusters=2, random_state=42)
clusters = kmeans.fit_predict(df_scaled)
\end{lstlisting}

\section{County-Level Snapshots}
\label{appendix:county_snapshots}

\begin{table}[H]
\centering
\caption{Sample of County-Level Indicator Values}
\label{tab:county_snapshot}
\begin{tabularx}{\textwidth}{l *{5}{>{\centering\arraybackslash}X}}
\toprule
\textbf{County} & \textbf{Bach\%} & \textbf{Pov\%} & \textbf{Unemp\%} & \textbf{Population} & \textbf{Cluster} \\
\midrule
Baxter   & 13.2 & 16.4 & 4.2 & 41,093 & Moderate \\
Izard    & 9.3  & 22.7 & 4.6 & 13,916 & High \\
Searcy   & 8.9  & 24.1 & 4.9 & 7,881  & High \\
Van Buren & 11.1 & 19.8 & 4.4 & 15,796 & Moderate \\
\bottomrule
\end{tabularx}
\end{table}
\section{Methodological Notes}
\label{appendix:method_notes}

This analysis followed the CRISP-DM methodology, including the following major phases:
\begin{itemize}
    \item Business understanding (regional education/employment needs)
    \item Data acquisition from public sources (ACS, BLS, NTIA)
    \item Data cleaning and harmonization (FIPS alignment, column renaming)
    \item Feature engineering (Min-Max normalization, composite indices)
    \item Modeling (KMeans clustering, Random Forest classification)
    \item Evaluation (accuracy, silhouette score, feature importance)
\end{itemize}

All scripts were implemented in Python and executed in a modular Jupyter notebook structure to ensure reproducibility.
\section{North Central Arkansas Counties}
\label{appendix:nca_counties}

\begin{table}[H]
\centering
\caption{North Central Arkansas (NCA) Counties Used for Regional Comparison}
\label{tab:nca_appendix}
\begin{tabular}{ll}
\toprule
