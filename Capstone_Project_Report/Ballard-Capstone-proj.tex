...
To uncover latent groupings of counties based on shared socio-economic characteristics, unsupervised learning was employed using the KMeans clustering algorithm. This method partitions observations into non-overlapping clusters by minimizing intra-cluster variance. The primary goal was to identify distinct regional profiles of educational and economic vulnerability across Arkansas.

\subsection{Methodology and Feature Selection}
The clustering process relied on scaled versions of the four core indicators: \texttt{BachelorsDegreePct}, \texttt{HighSchoolGradPct}, \texttt{PovertyRate}, and \texttt{UnemploymentRate}. Prior to modeling, all values were normalized to a 0--1 range using \texttt{MinMaxScaler}. A KMeans algorithm was initialized using \texttt{sklearn}'s implementation, with cluster range evaluated from $k = 2$ to $k = 10$.

To determine the optimal number of clusters, both the Elbow Method and Silhouette Coefficient were calculated. The latter provides a score between $-1$ and $+1$ indicating how well-separated each point is from other clusters.

\begin{lstlisting}[language=Python, caption={KMeans Clustering with Silhouette Evaluation}]
...
\subsection{Cluster Evaluation and Selection}

A silhouette analysis revealed that the optimal number of clusters was $k = 3$, balancing interpretability and model stability. Counties were assigned cluster labels using the fitted model, and summary statistics were computed for each group to support interpretation.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth, trim=20 20 20 20, clip]{Capstone_Project_Report/images/maps/clustering_evaluation.png}
\caption{Silhouette Scores Across Cluster Values ($k$): A three-cluster model achieved the highest silhouette score, indicating well-defined groupings.}
\label{fig:silhouette}
\end{figure}
\vspace{-1em}
\FloatBarrier

\subsection{Cluster Profiles and Map}
Each cluster was profiled based on the mean values of its component counties. The results revealed three interpretable segments:
\begin{itemize}
  \item \textbf{Cluster 0 (High Vulnerability):} Low educational attainment, high poverty and unemployment
  \item \textbf{Cluster 1 (Moderate Risk):} Mixed indicators with moderate education and economic pressures
  \item \textbf{Cluster 2 (Low Vulnerability):} Higher educational attainment and lower economic distress
\end{itemize}

A choropleth map was generated to visualize the geographic distribution of clusters across Arkansas.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth, trim=20 20 20 20, clip]{Capstone_Project_Report/images/maps/arkansas_cluster_map_comprehensive.png}
\caption{KMeans Cluster Assignments for Arkansas Counties ($k=3$): Cluster 0 counties (red) show higher structural vulnerability, while Cluster 2 counties (green) exhibit relative advantage.}
...
Summary statistics were generated for each cluster to provide descriptive insights into their defining characteristics.

\begin{table}[H]
\centering
\caption{Cluster Means by Key Indicators}
\label{tab:cluster_summary}
\small
\begin{tabularx}{\textwidth}{l *{4}{>{\centering\arraybackslash}X}}
\toprule
\textbf{Cluster} & \textbf{Bachelors\%} & \textbf{Poverty\%} & \textbf{Unemp\%} & \textbf{Population} \\
\midrule
Cluster 0 (High Risk) & 10.5 & 26.1 & 5.9 & 12,500 \\
Cluster 1 (Mid Risk)  & 16.1 & 17.8 & 4.1 & 24,000 \\
Cluster 2 (Low Risk)  & 22.4 & 12.3 & 3.2 & 44,000 \\
\bottomrule
\end{tabularx}
\end{table}

These clusters formed the basis for supervised classification modeling in the next section, enabling prediction of county-level vulnerability categories from known features.
