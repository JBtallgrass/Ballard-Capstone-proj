...
Exploration of educational attainment revealed considerable variability in the percentage of adults with a high school diploma versus those with a bachelor’s degree. Using derived metrics like \texttt{EducationGap}, most counties exhibited gaps greater than 30 percentage points. Several counties had \texttt{BachelorsDegreePct} values under 15\%, activating a binary flag indicating a potential access or attainment issue.

Pair plots and correlation matrices showed strong inverse relationships between bachelor’s degree attainment and both poverty and unemployment rates. In other words, counties with higher educational attainment had better economic outcomes, reinforcing the central hypothesis that education plays a critical role in rural development.
...
Visuals such as the correlation heatmap and annotated boxplots provided not only quantitative validation of feature relationships but also qualitative cues about which counties might warrant closer policy attention. The exploratory phase concluded by laying the groundwork for the modeling and clustering analyses in subsequent sections, where these insights were operationalized into interpretable scores and groupings.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{figures/ar_section6_1_kmeans_clusters.png}
\caption{Clustered Counties by Poverty Rate and Bachelor's Degree Attainment}
\label{fig:kmeans_clusters_scatter}
\end{figure}


Feature importance scores from the Random Forest model revealed that \textbf{unemployment rate} was the most influential predictor of cluster membership, followed by \textbf{high school graduation rate} and the derived \textbf{education gap} between high school and bachelor's attainment. Together, these three features accounted for nearly 70\% of the model’s decision-making power. Interestingly, \textbf{poverty rate} and \textbf{bachelor’s degree attainment} played a relatively smaller role, suggesting that broader educational coverage and workforce participation rates were more decisive in shaping regional profiles than degree completion alone.
